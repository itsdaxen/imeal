-- A recipe can contain a kitchen instruction rather than a product (for example,
-- reserved pasta water). The reviewed AI proposal accounts for that source explicitly
-- as omitted, and applying it removes only that exact row from the selected list.
